...
任何位置参数都可以写成 **`N%`**，执行前按指令轴向换算为像素：

| 参数类型 | 换算方式 |
|---|---|
| X 坐标 | `N% × (宽-1)`（`100%` = 最后一列像素） |
| Y 坐标 | `N% × (高-1)` |
| 半径 / 尺寸（单一值） | `N% × max(宽,高)` |

`poly` 的顶点对从颜色（或模式）之后开始按 X、Y 交替换算。非坐标参数（颜色、模式、
强度、角度、密度等）不参与百分比换算。

```
size 256 256
ellipse 50% 50% 35% 30% c9 fill   # 居中椭圆，半径按长边
rect 0% 0% 100% 100% c1 stroke    # 整幅边框
```

> 大画布强烈建议用百分比：模型无需心算坐标，指令遵循更稳定。
> 非方形画布上 X/Y 各按自身维度换算。
...
| 1.7 | 画布上限 512 → **2048**；位置参数支持**百分比坐标**（`50%`）；新增平滑超分导出（`core/supersample.js`） |
