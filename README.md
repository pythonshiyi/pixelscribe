...
| 特性 | 说明 |
|---|---|
| **PixelScript DSL** | 33 条紧凑绘图指令，一行一条，模型一次写对 |
...
33 条指令，覆盖点、线、矩形、圆角矩形、圆、椭圆、多边形、洪水填充、
全图换色、描边、渐变、抖动、噪声、拷贝、翻转、旋转、明暗调整、位图文字，
以及精细化的贝塞尔曲线 `curve`、圆弧 `arc`、同色系 `shade`、立体浮雕 `bevel`、
抖动渐变 `graddither`。
...
└─ test/
   ├─ selftest.mjs          单元 / 集成自测（158 项，含真实 HTTP 闭环）
