...
| **补间 / 缓动** | 关键帧之间一键补间：16 种缓动曲线（含回弹/弹跳/定格）；**AI 补间**请模型补出合理运动姿态，失败自动回退交叉溶解 |
...
│     │                     supersample（平滑超分） easing（缓动） tween（补间） audio（音频/拍点）
│     ├─ lang/              compiler（解析+执行） + font5x7
│     ├─ ai/                provider（流式+视觉） prompts demo agent（闭环+导演+补间） multiagent（多智能体）
│     └─ ui/                dom app tools panels chat gallery
└─ test/
   ├─ selftest.mjs          单元 / 集成自测（259 项，含缓动/补间/音频/ SVG/多智能体与真实 HTTP 视觉闭环）
...
**补间**：点 `⇢` 选择起始/结束关键帧、中间帧数与缓动曲线（16 种，含回弹/弹跳/定格）。
勾选「AI 补间」会请模型参考 A/B 两帧补出更合理的运动姿态，模型不可用时自动回退为交叉溶解。
...
```bash
npm test           # 单元 + 集成 + 无头 UI 冒烟（共 333 项）
...
**缓动曲线 / 帧间补间 / 音频峰值与拍点 / SVG 矢量写出 / 多智能体分工**，
...
脚本运行、全部模态框、AI 演示闭环、补间与音频控件、SVG 导出、
localStorage 持久化、PNG 导出。
